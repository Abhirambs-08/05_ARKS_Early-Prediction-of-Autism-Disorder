...
selected = option_menu(
    menu_title=None,
    options=["Home", "Image Analysis", "Questionnaire","Autism Chatbot","Compute two"],
    icons=["house","book","envelope","chat-dots-fill","book"],
    menu_icon="cast",
    default_index=0,
...
def camera_section():
    ...
    if uploaded_image is not None:
        ...
        print(result)
        #img_score = result[0][1]
        st.session_state.img_score = result[0][1]
...
def questionnaire_section():
    ...
    questionnaire['Q8'] = st.radio("Does he not show an interest in gathering knowledge abou understanding people's feelings and thoughts from their facial expressions?", ["No", "Yes"])
    questionnaire['Q10'] = st.radio("Does he find it chalt different topics or subjects?", ["No", "Yes"])
    questionnaire['Q9'] = st.radio("Is he not good atlenging to infer people's intentions?",["No", "Yes"])
    ...
    st.session_state.quest_score = autism_percentage
    # ... (rest of the code)
...
def compute_two():
    ...
    finalsubmit = st.button("Submit finally")
    if finalsubmit:
        result = (autism_percentage+result[0][0])/2.0
        st.write(f"The probability percentage for being Autistic is : {result*100}%")
